...
namespace Q1FileManager.View
{
    public class ConsoleButtonPanel : AConsolePanel, IPanel
    {
        ...
        public ConsoleButtonPanel(int width, int height, int left, int top)
        {
            ...
            _menu.Add(ConsoleKey.F5, "Копия");
            _menu.Add(ConsoleKey.F6, "Перемещ");
            _menu.Add(ConsoleKey.F7, "Создать");
            _menu.Add(ConsoleKey.F8, "Переимен");
            _menu.Add(ConsoleKey.F9, "Удаление");
        ...
        public void Show()
        {
            ...
            foreach (var btn in _menu)
            {
                ...
                btnText.Insert(0, emptySpace > 0 ? new string(' ', emptySpace/2 + (emptySpace % 2 == 1 ? 1 : 0)) : String.Empty);
                btnText.Append(emptySpace > 1 ? new string(' ', emptySpace / 2) : String.Empty);
